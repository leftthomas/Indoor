...
public class composerLayout extends RelativeLayout {
	...
	public void init(int[] imgResId, int showhideButtonId, int crossId,
		...
		if (pCode == CENTERBOTTOM || pCode == CENTERTOP) {
			...
			if (thislps.width != -1
                ...
                /* 因為animation嘅setInterpolator設咗OvershootInterpolator，
				即系喐到目標之後仍然行多一段（超過目標位置）~然後再縮返到目標位置，
				所以父layout就要再放大少少。而因為呢個OvershootInterpolator接納嘅
				係一個彈力（浮點）值，佢經過一定算法計算出個時間……如果要根據呢個彈
				力轉換做距離數值，就比較麻煩，所以我只系求其加咗1/10個半徑。想追求
				完美嘅~可以自行研究下OvershootInterpolator類同Animation類，
				http://www.oschina.net可以揾倒android*/
                // sdk嘅源碼。
		...
		cross.startAnimation(myAnimations.getRotateAnimation(0, 360, duretime));
		this.addView(rl1);
	...
	public void expand() {
		myani.startAnimationsIn(duretime);
		cross.startAnimation(myAnimations.getRotateAnimation(0, -270, 200));
