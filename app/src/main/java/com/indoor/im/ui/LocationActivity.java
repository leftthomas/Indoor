...
public class LocationActivity extends BaseActivity implements
    ...
    private void initBaiduMap() {
        ...
        if (type.equals("select")) {// 选择发送位置
            initTopBarForBoth("位置", R.drawable.base_action_bar_true_bg_n, new onRightImageButtonClickListener() {
    ...
    private void gotoChatPage() {
        if (lastLocation != null) {
            ...
            showTag("获取地理位置信息失败!", Effects.slideOnTop, R.id.bmapView);
        }
    }
...
//		 开启定位图层
    ...
    public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
        ...
        if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
            showTag("抱歉，未能找到结果", Effects.slideOnTop, R.id.bmapView);
            return;
        }
    ...
    public class BaiduReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            if (s.equals(SDKInitializer.SDK_BROADTCAST_ACTION_STRING_PERMISSION_CHECK_ERROR)) {
                showTag("key 验证出错! 请在 AndroidManifest.xml 文件中检查 key 设置", Effects.slideOnTop, R.id.bmapView);
            } else if (s
                    .equals(SDKInitializer.SDK_BROADCAST_ACTION_STRING_NETWORK_ERROR)) {
                showTag("网络出错", Effects.slideOnTop, R.id.bmapView);
            }
        }
    }
