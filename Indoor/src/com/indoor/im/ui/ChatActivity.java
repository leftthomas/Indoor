...
import com.indoor.im.adapter.EmoViewPagerAdapter;
import com.indoor.im.adapter.EmoteAdapter;
import com.indoor.im.adapter.MessageChatAdapter;
import com.indoor.im.adapter.RecordPlayClickListener;
import com.indoor.im.bean.FaceText;
import com.indoor.im.bean.User;
import com.indoor.im.config.BmobConstants;
import com.indoor.im.util.CommonUtils;
import com.indoor.im.util.FaceTextUtils;
import com.indoor.im.view.EmoticonsEditText;
...
//				+ targetId);
	...
	private void initRecordManager(){
		...
		// 设置音量大小监听--在这里开发者可以自己实现：当剩余10秒情况下给用户的提示，类似微信的语音那样
	...
	private void initView() {
		...
		ImageView backp=(ImageView) findViewById(R.id.ic);
		User user=BmobUser.getCurrentUser(this,User.class);
		String uri=user.getWallpaperUri();
		DisplayMetrics metric = new DisplayMetrics();  
		getWindowManager().getDefaultDisplay().getMetrics(metric);  
		int width = metric.widthPixels;     // 屏幕宽度（像素）  
		int height = metric.heightPixels;   // 屏幕高度（像素）  
		android.view.ViewGroup.LayoutParams para;  
	    para = backp.getLayoutParams();  
	    para.height = height;  
	    para.width = width;  
	    backp.setLayoutParams(para); 
	    ImageLoader.getInstance().displayImage(uri, backp);
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				if (!CommonUtils.checkSdCard()) {
					showTag("发送语音需要SD卡支持！",Effects.scale,R.id.chat);
					return false;
				}
				try {
					v.setPressed(true);
	...
	public void showResendDialog(final View parentV, View v, final Object values) {
        ...
        dialogBuilder
                ...
                .withMessage("确定重发该消息?")                             //.withMessage(null)  no Msg
	...
	public void onClick(View v) {
		...
		case R.id.btn_chat_send:// 发送文本
			...
			if (msg.equals("")) {
				showTag("请输入发送消息!",Effects.scale,R.id.chat);
				return;
			}
			boolean isNetConnected = CommonUtils.isNetworkAvailable(this);
			if (!isNetConnected) {
				showTag("当前网络不可用,请检查您的网络!",Effects.scale,R.id.chat);
			}
			// 组装BmobMessage对象
		...
		if (resultCode == RESULT_OK) {
				...
				if (data != null) {
					...
					if (selectedImage != null) {
						...
						if (localSelectPath == null
							...
							showTag("找不到您想要的图片",Effects.scale,R.id.chat);
							return;
						}
			...
			case BmobConstants.REQUESTCODE_TAKE_LOCATION:// 地理位置
				...
				if (address != null && !address.equals("")) {
					...
					showTag("无法获取到您的位置信息!",Effects.scale,R.id.chat);
				}
	...
	protected void onPause() {
		...
		if (RecordPlayClickListener.isPlaying
				&& RecordPlayClickListener.currentPlayListener != null) {
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (msg.what == NEW_MESSAGE) {
				...
				BmobMsg m = BmobChatManager.getInstance(ChatActivity.this).getMessage(message
						.getConversationId(), message.getMsgTime());
	...
	public void onRefresh() {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				if (total <= currents) {
					showTag("聊天记录加载完了哦!",Effects.scale,R.id.chat);
				} else {
